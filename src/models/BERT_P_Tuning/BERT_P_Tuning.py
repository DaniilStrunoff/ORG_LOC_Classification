...
MODEL_NAME: str = "bert_ptune"


class BERTPTuningTrainConfig(BaseTrainConfig):
    model_name: Literal["bert_ptune"] = MODEL_NAME  # type: ignore
    num_virtual_tokens: int = Field(1, title="Num virtual tokens", ge=1, le=10000)
    encoder_hidden_size: int = Field(128, title="Encoder hidden size", ge=1, le=10000)
    epochs: int = Field(500, title="Epochs", ge=1, le=10000)
...
class BERT_P_Tuning(IModel[BERTPTuningTrainConfig]):
    ...
    def train(
        ...
        self.model = get_peft_model(self.base, peft_cfg).to(self.device)
        ...
        for name, p in self.model.named_parameters():
            if "prompt_" in name or "modules_to_save" in name:
        ...
        for e in range(1, config.epochs + 1):
            ...
            if val_auc > best_auc + 1e-4:
                ...
                for k, v in self.model.state_dict().items():
                    if k.startswith("base_model.classifier.modules_to_save."):
                        peft_sd[k] = v.detach().cpu()
...
ModelsFactory.register_model(MODEL_NAME, BERT_P_Tuning)
